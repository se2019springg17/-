...
function _alert_location($_info,$_url){//提示信息
	echo "<script type='text/javascript'>alert('$_info');window.location.href='{$_url}';</script>";
	exit();
}
function _check_username($_string,$_min_num,$_max_num){//检查用户名
	$_string = trim($_string);
	if (mb_strlen($_string,'utf-8')<$_min_num || mb_strlen($_string,'utf-8')>$_max_num)
	_alert_back('用户名不符合');
	return $_string;
}
function _check_tel($_string,$_min_num,$_max_num){//检查电话
	$_string = trim($_string);
	if (mb_strlen($_string,'utf-8')<$_min_num || mb_strlen($_string,'utf-8')>$_max_num)
	_alert_back('手机号需要是11位数字');
	return $_string;
}
function _check_password($_password,$_repassword,$_min_num){//检查密码，用于修改时
